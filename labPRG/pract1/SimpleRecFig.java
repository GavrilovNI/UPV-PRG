...
public class SimpleRecFig {
     ...
     * with decreasing sides. The largest square has side length s, 
     * and the rest of the squares they reduce their side length by half. 
     * @param g Graph2D, the drawing canvas, an object of the class Graph2D.
     * @param ord int, number of squares.
     * @param s double, side length of the largest square.
     ...
     * PRECONDITION: ord >= 1 
     */
    private static void recFig(Graph2D g, int ord, double s, double xSL, double ySL) {
        // Base case, ord == 1, a square of side length s is drawn.
        // Recursive step, ord > 1, a square of side length s is drawn, and a 
        // subfigure of ord - 1 squares is drawn.
        if (ord == 1) {
            delay();
            g.drawRect(xSL, ySL, s, s, Color.BLUE, 2);
        } else {
            delay();
            g.drawRect(xSL, ySL, s, s, Color.BLUE, 2);
            recFig(g, ord - 1, s / 2, xSL - s / 2, ySL + s / 2);
        }
    } 
     ...
     * PRECONDITION: ord >= 1 
     */
    private static void iterFig(Graph2D g, int ord, double s, double xSL, double ySL) {
        for (int i = ord; ord >= 1; ord--) {        
            g.drawRect(xSL, ySL, s, s, Color.RED, 2);
            delay();
            xSL = xSL - s / 2;
            ySL = ySL + s / 2;
            s = s / 2;
